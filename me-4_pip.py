...
#    print cfg.TEST.SCALES
    # Load the demo image
    im1_file = os.path.join(data_dir,'color', 'I'+ image_name + '.jpg')
    im2_file = os.path.join(data_dir,'thermal', 'I'+ image_name + '.jpg')
    print im1_file
    print im2_file
...
def f():
    if (int(i)%10)==0:
        ...
        data_dir = os.path.join(cfg.ROOT_DIR, 'data/demo_pedestrian/set08_V001')            
        im=format(i, '05d')
        trackers = cv2.MultiTracker_create()    
        gt = load_kitti_annotation(data_dir, im)       
        det = demo_detection(net, data_dir, im, CLASSES, gt)        
        tracker = OPENCV_OBJECT_TRACKERS["csrt"]()
        trackers.add(tracker, im, bound)
        plt.show()
...
def g(bound):
    ...
    if (int(i)%10)!=0:  
        data_dir = os.path.join(cfg.ROOT_DIR, 'data/demo_pedestrian/set08_V001')            
        im=format(i, '05d')
        im = os.path.join(data_dir,'color', 'I'+im+'.jpg')              
        im = cv2.imread(im)
...
#    train_scale = 600
    ...
    for i in range (0,2700,10):
        bound = Queue()
        p = Process(target=f, args=())
        h = Process(target=g, args=(bound,))
        p.start()
        h.start()
